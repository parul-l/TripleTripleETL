...
class TestClosestToBallETL(unittest.TestCase):
    ...
    def test_get_queries(self):
        ...
        path = (
            ...
            'closest_to_ball_rank_athena_to_s3parquet.open'
        )
        with mock.patch(path, open_mock):   
            ...
            etl.get_queries(
                sql_path_balldist='sql1.sql',
                sql_path_closest_to_ball='sql2.sql'
            )

            self.assertEqual(
                first=open_mock.call_count,
                second=2
            )
    ...
    def test_create_tmp_tables(self):
        ...
        path = (
            'triple_triple_etl.load.storage.'
            'closest_to_ball_rank_athena_to_s3parquet'
        )
        with mock.patch.multiple(path, **patches):   
            ...
            self.assertEqual(
                first=check_table_exists_mock.call_count,
                second=2
            )

    def test_move_closest_to_ball_tmp_to_final(self):
        ...
        with mock.patch.multiple(path, **patches):   
            ...
            etl.move_closest_to_ball_tmp_to_final()
    ...
    def test_drop_tmp_tables(self):
        ...
        path = (
            'triple_triple_etl.load.storage.'
            'closest_to_ball_rank_athena_to_s3parquet.execute_athena_query'
        )
        with mock.patch(path, execute_athena_query_mock):
            etl = ClosestToBallETL(
                season='2015-2016',
                gameid_bounds=['0010', '0020'],
            )
    ...
    def test_cleanup(self):
        ...
        get_bucket_content_mock.side_effect = [
            ['ball_dist/somekey/season=2015-2016/gameid=1234/file1',
             'ball_dist/somekey/season=2015-2016/gameid=9876/file2'],
            ['closest/somekey/season=2015-2016/gameid=1234/file1',
             'closest/somekey/season=2015-2016/gameid=9876/file2'],
        ]
        remove_bucket_contents_mock = mock.Mock(return_value=1)
        get_uploaded_metadata_mock = mock.Mock(return_value=copy.deepcopy(mock_df_uploaded))
        ...
        with mock.patch.multiple(path, **patches):
            ...
            # check remove_bucket_contents called appropriate times (4 files balldist/closest)
            self.assertEqual(
    ...
    def test_run(self):
        get_queries_mock = mock.Mock()
        create_tmp_tables_mock = mock.Mock()
        move_closest_to_ball_tmp_to_final_mock = mock.Mock(return_value=['some_list'])
        alter_table_mock = mock.Mock()
        drop_tmp_tables_mock = mock.Mock()
        ...
        etl.get_queries = get_queries_mock
        etl.create_tmp_tables = create_tmp_tables_mock
        etl.move_closest_to_ball_tmp_to_final = move_closest_to_ball_tmp_to_final_mock
        etl.alter_table = alter_table_mock
        etl.drop_tmp_tables = drop_tmp_tables_mock
        ...
        etl.get_queries.assert_called_once_with()
        etl.create_tmp_tables.assert_called_once_with()
        etl.move_closest_to_ball_tmp_to_final.assert_called_once_with()
        etl.alter_table.assert_called_once_with(move_closest_to_ball_tmp_to_final_mock.return_value)
        etl.drop_tmp_tables.assert_called_once_with()
