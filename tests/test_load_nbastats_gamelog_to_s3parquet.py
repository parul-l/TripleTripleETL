...
class TestHelpers(unittest.TestCase):
    def test_first_last_date_season(self):
        ...
        path = 'triple_triple_etl.load.storage.nbastats_gamelog_to_s3parquet.get_data'
    ...
    def test_append_upload_metadata(self):
        ...
        df_upload_mock = pd.DataFrame([list('efghijklm')], columns=upload_cols)
        ...
        self.assertEqual(first=set(df_result.params), second={'m', 'mock-params'})
        self.assertEqual(first=set(df_result.game_id), second={'g', 'c'})


class TestNBAStatsGameLogsS3ETL(unittest.TestCase):
    """Tests for nbastats_gamelog_to_s3parquet.py"""
    
    def test_metadata(self):    
        # mock functions
        get_upload_metadata_mock = mock.Mock(return_value=mock_df_uploaded)
        
        path = 'triple_triple_etl.load.storage.nbastats_gamelog_to_s3parquet.get_uploaded_metadata'
    ...
    def test_cleanup(self):
        ...
        # mock functions
        etl.df_uploaded = mock.Mock()
        etl.df_uploaded.to_parquet = mock.Mock()

        path = 'triple_triple_etl.load.storage.nbastats_gamelog_to_s3parquet.shutil'
